...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        NSArray *images = @[@"left",@"right",@"vertical",@"horizontal"];
//        NSArray *titles = @[@"滤镜",@"滤镜",@"滤镜",@"滤镜",@"滤镜"];
        HCPhotoEditBaseScrollView *scrollView = [[HCPhotoEditBaseScrollView alloc] initWithFrame:self.bounds bottomTitle:@"" imagesNameArray:images titlesArray:nil buttonWidth:self.bounds.size.width/4.0 imageSize:30];
...
+(HCPhotoEditBaseItemView*)showInView:(UIView*)view
{
    ...
    HCPhotoEditRotateItemView *effectView = [[HCPhotoEditRotateItemView alloc] initWithFrame:CGRectMake(0, view.bounds.size.height - 150 - offset, view.bounds.size.width, 150 + offset)];
