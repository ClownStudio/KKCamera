...
//  PhotoX
//
//  Created by Leks on 2017/12/20.
//  Copyright © 2017年 idea. All rights reserved.
//

#import "PhotoXAcvFilter.h"

@interface PhotoXGPUImageACVFile : NSObject{
    short version;
    short totalCurves;
...
unsigned short int16WithBytesPhotoX(Byte* bytes);
@end

@implementation PhotoXGPUImageACVFile
...
- (id) initWithACVFileData:(NSData *)data {
    ...
    if (self != nil)
    {
        ...
        version        = int16WithBytesPhotoX(rawBytes);
        rawBytes+=2;
        
        totalCurves    = int16WithBytesPhotoX(rawBytes);
        rawBytes+=2;
        ...
        {
            unsigned short pointCount = int16WithBytesPhotoX(rawBytes);
            rawBytes+=2;
            ...
            {
                unsigned short y = int16WithBytesPhotoX(rawBytes);
                rawBytes+=2;
                unsigned short x = int16WithBytesPhotoX(rawBytes);
                rawBytes+=2;
...
NSString *const kPhotoXGPUImageToneCurveFragmentShaderString = SHADER_STRING
(
 ...
 void main()
 {
     ...
     lowp float redCurveValue = texture2D(toneCurveTexture, vec2(textureColor.r, 0.0)).r * mixturePercent;
     lowp float greenCurveValue = texture2D(toneCurveTexture, vec2(textureColor.g, 0.0)).g * mixturePercent;
     lowp float blueCurveValue = texture2D(toneCurveTexture, vec2(textureColor.b, 0.0)).b * mixturePercent;
     
     gl_FragColor = vec4(redCurveValue, greenCurveValue, blueCurveValue, textureColor.a);
 }
 );

@interface PhotoXAcvFilter()
{
...
@implementation PhotoXAcvFilter
...
- (id)init;
{
    if (!(self = [super initWithFragmentShaderFromString:kPhotoXGPUImageToneCurveFragmentShaderString]))
    {
        return nil;
    }
...
- (id)initWithACVData:(NSData *)data {
    ...
    PhotoXGPUImageACVFile *curve = [[PhotoXGPUImageACVFile alloc] initWithACVFileData:data];
...
{
    NSData* fileData = [NSData dataWithContentsOfURL:curveFileURL];
    PhotoXGPUImageACVFile *curve = [[PhotoXGPUImageACVFile alloc] initWithACVFileData:fileData];
