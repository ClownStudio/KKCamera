...
-(instancetype)initWithFrame:(CGRect)frame  bottomTitle:(NSString*)title  imagesArray:(NSArray*)images  titlesArray:(NSArray*)titles buttonWidth:(float)width  imageSize:(float)size
{
    ...
    if (self) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            {
                HCPhotoEditCustomButton *btn = [[HCPhotoEditCustomButton alloc] initWithImage:images[index] highlightedImage:images[index] title:titles[index] font:8 imageSize:size];
                btn.tag = index + 10;
...
//                NSString *title = (index < titles.count) ? titles[index]: nil;
                ...
                HCPhotoEditCustomButton *btn = [[HCPhotoEditCustomButton alloc] initWithImage:IMAGE_WITHNAME(name) highlightedImage:hightImage title:titles[index] font:8 imageSize:size];
                btn.tag = index + 10;
